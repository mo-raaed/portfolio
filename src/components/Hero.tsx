      ...
      <div className="max-w-6xl mx-auto px-6 md:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-7 space-y-8">
            <p className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-border bg-surface text-xs font-semibold text-accent tracking-[0.15em] uppercase">
            ...
            <h1 className="font-display font-semibold text-5xl md:text-7xl lg:text-8xl tracking-tight leading-[1.04] text-foreground">
              <span className="block">From neural networks</span>
          ...
          {/* Right: portrait with offset burgundy frame */}
          <div className="lg:col-span-5">
            <div className="relative group max-w-sm mx-auto lg:max-w-none lg:mr-4">
              <div
                aria-hidden="true"
                className="absolute inset-0 translate-x-3 translate-y-3 rounded-lg border-2 border-accent/50"
              />
              <div className="relative rotate-2 group-hover:rotate-1 transition-transform duration-500 ease-out">
                <SmartImage
                  src="/images/me.jpg"
          ...
          <dl className="mt-16 md:mt-20 border-t border-border pt-8 grid grid-cols-2 md:grid-cols-4 gap-y-8 md:divide-x md:divide-border">
            {stats.map(stat => (
              <div key={stat.label} className="md:px-8 md:first:pl-0 md:last:pr-0">
                ...
                <dt className="text-xs font-medium uppercase tracking-wider text-muted-foreground mt-1.5">
                  {stat.label}
                </dt>
              </div>
            ))}
